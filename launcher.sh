...
if [ "$(id -u)" = "0" ]; then
  clear
  echo -e "\n$(ired " !!! Please do not run this script with sudo or as the ROOT user.") "
  echo -e "\nLaunch with $(green "bash launcher.sh") as a REGULAR linux user instead"
  echo -e "Give executable permissions with $(green "chmod +x launcher.sh") and hit $(green "ENTER")"
  echo -e "Then give it a go -- $(green "./launcher.sh")\n"
  exit 1
fi
...
if [ $(systemd-detect-virt -c) != "none" ]; then
  ...
  echo "$(ired "                                                                                 ")"
  echo "$(ired "        Running BenchCore on a container-based system is not recommended!        ")"
...
LOC_SERVER="http://localhost:6620"
...
SNAPURL="https://img.benchcore.io/current"
...
function proc_vars {
  node=`pgrep -a "node" | grep benchcore-v1 | awk '{print $1}'`
  if [ "$node" == "" ] ; then
    node=0
  fi
  ...
  # Looking for benchcore-v1 installations and performing actions
  benchcoreDir=`locate -b "\benchcore-v1"`

  # Getting the parent of the install path
  parent=`dirname $benchcoreDir 2>&1`
...
#PSQL Queries
query() {
  PUBKEY="$(psql -d benchcore_bex -t -c 'SELECT ENCODE("publicKey",'"'"'hex'"'"') as "publicKey" FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
  DNAME="$(psql -d benchcore_bex -t -c 'SELECT username FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
  PROD_BLOCKS="$(psql -d benchcore_bex -t -c 'SELECT producedblocks FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
  MISS_BLOCKS="$(psql -d benchcore_bex -t -c 'SELECT missedblocks FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
  #BALANCE="$(psql -d benchcore_bex -t -c 'SELECT (balance/100000000.0) as balance FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | sed -e 's/^[[:space:]]*//')"
  BALANCE="$(psql -d benchcore_bex -t -c 'SELECT to_char(("balance"/100000000.0), '"'FM 999,999,999,990D00000000'"' ) as balance FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
  FORGED="$(psql -d benchcore_bex -t -c 'SELECT to_char((("fees" + "rewards")/100000000.0), '"'FM 999,999,999,990D00000000'"' ) as total_forged FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
  HEIGHT="$(psql -d benchcore_bex -t -c 'SELECT height FROM blocks ORDER BY HEIGHT DESC LIMIT 1;' | xargs)"
  RANK="$(psql -d benchcore_bex -t -c 'WITH RANK AS (SELECT DISTINCT "publicKey", "vote", "round", row_number() over (order by "vote" desc nulls last) as "rownum" FROM mem_delegates where "round" = (select max("round") from mem_delegates) ORDER BY "vote" DESC) SELECT "rownum" FROM RANK WHERE "publicKey" = '"'"$PUBKEY"'"';' | xargs)"
}
...
turn() {
  ...
  while true; do
    ...
    echo -e "$(green "      BEX Balance      : ")$(yellow "$BALANCE")"
    echo
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
    if [ -e $benchcoreDir/app.js ]; then
      echo -e "\n$(green "       ✔ BenchCore Node installation found!")\n"
      if [ "$node" != "" ] && [ "$node" != "0" ]; then
        echo -e "$(green "      BenchCore Node process is running with:")"
        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
        echo -e "$(green "   and Work Directory: $benchcoreDir")\n"
      else
        echo -e "\n$(red "       ✘ No BenchCore Node process is running")\n"
      fi
    else
      echo -e "\n$(red "       ✘ No BenchCore Node installation is found")\n"
    fi
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
...
function stats {
  ...
  if [ "$node" != "" ] && [ "$node" != "0" ]; then
    echo -e "$(green "       Instance of BenchCore Node found with:")"
    echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
    echo -e "$(green "       Directory: $benchcoreDir")\n"
  else
    echo -e "\n$(red "       ✘ BenchCore Node process is not running")\n"
    pause
  fi
}
...
function ntpd {
  ...
  if [ $(systemd-detect-virt) == "lxc" ] || [ $(systemd-detect-virt) == "openvz" ]; then
    ...
    if ! sudo pgrep -x "ntpd" > /dev/null; then
      ...
      sleep 2
      if ! sudo pgrep -x "ntpd" > /dev/null; then
        echo -e "NTP failed to start! It should be installed and running for BenchCore.\n Check /etc/ntp.conf for any issues and correct them first! \n Exiting."
        exit 1
      fi
...
# Logrotate for BenchCore Node logs
function log_rotate {
  if [[ "$(uname)" == "Linux" ]]; then

    if [ ! -f /etc/logrotate.d/benchcore-logrotate ]; then
      echo -e " Setting up Logrotate for BenchCore node log files."
      sudo bash -c "cat << 'EOF' >> /etc/logrotate.d/benchcore-logrotate
$benchcoreDir/logs/benchcore.log {
        size=50M
        copytruncate
...
function git_upd_check {
  if [ -d "$benchcoreDir" ]; then
    ...
    if [ "$LOCAL" == "$REMOTE" ]; then
      echo -e "         $(igreen "    BenchCore Node is Up-to-date    \n")"
      UP_TO_DATE=1
...
function purge_pgdb {
  if [ $(dpkg-query -W -f='${Status}' postgresql } 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
    ...
    echo -e "    $(ired "   running BenchCore Node processes and will  ")"
  ...
  if [ "$(ls -A $SNAPDIR)" ]; then
    ...
    if [[ "${REPLY}" =~ $re ]]; then
      ...
      if [ $REPLY -le ${#snapshots[*]} ]; then
        ...
        pg_restore -O -j 8 -d benchcore_bex $SNAPDIR/${snapshots[$(($REPLY-1))]} 2>/dev/null
    ...
    if [[ $? -eq 0 ]]; then
      ...
      if [[ "$YN" =~ [Yy]$ ]]; then
        ...
        pg_restore -O -j 8 -d benchcore_bex $SNAPDIR/current 2>/dev/null
...
# Install BenchCore Node
function inst_benchcore {
  # proc_vars
  cd $HOME
  mkdir benchcore-v1
  git clone https://github.com/benchcore/benchcore-v1.git 2>/dev/null
...
function create_db {
  ...
  sleep 1
  # sudo -u postgres dropdb --if-exists benchcore_bex
  # sleep 1
...
function db_exists {
  ...
  if [[ ! $(sudo -u postgres psql benchcore_bex -c '\q' 2>&1) ]]; then
...
function drop_db {
  ...
  dropdb --if-exists benchcore_bex
}
...
function update_benchcore {
  if [ "$UP_TO_DATE" -ne 1 ]; then
    ...
    echo "BenchCore Node is already up to date!"
    sleep 2
  fi
}
...
function secret {
  ...
  #Put check if benchcoreDir is empty, if it is stays only config.mainnet.json
...
one() {
  ...
  if [ -e $benchcoreDir/app.js ]; then
    clear
    asciiart
    echo -e "\n$(green "       ✔ BenchCore Node is already installed!")\n"
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
      echo -e "$(green "A working instance of BenchCore Node is found with:")"
      echo -e "$(green "System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "and Work Directory: $benchcoreDir")\n"
    fi
    pause
    ...
    clear
    asciiart
    echo -e "$(yellow "           Installing BenchCore node....")"
    create_db
    inst_benchcore
    clear
    asciiart
    echo -e "$(green "          ✔ BenchCore node was installed")\n"
    sudo updatedb
    sleep 1
    proc_vars
    log_rotate
...
# Reinstall BenchCore Node
two() {
  clear
  asciiart
  echo -e "$(ired "!!! This option will erase your DB and BenchCore Node installation !!!")\n"
  ...
  if [ "$keys" == "Y" ]; then
    ...
    if [ -e $benchcoreDir/app.js ]; then
      clear
      asciiart
      echo -e "\n$(green " ✔ BenchCore Node installation found in $benchcoreDir")\n"
      if [ "$node" != "" ] && [ "$node" != "0" ]; then
        echo -e "$(green "A working instance of BenchCore Node is found with:")"
        echo -e "$(green "System PID: $node, Forever PID $forever_process")"
        echo -e "$(yellow "           Stopping BenchCore node ...")\n"
        cd $benchcoreDir
      ...
      if [ -e $parent/config.mainnet.json ] ; then
        ...
        if [ "$keys" == "Y" ]; then
          cp $benchcoreDir/config.mainnet.json $parent
          cd $parent
        fi
      else
        cp $benchcoreDir/config.mainnet.json $parent
      ...
      echo -e "$(yellow "        Removing BenchCore Node directory...")\n"
      sleep 1
      rm -rf $benchcoreDir
      drop_db
      drop_user
      ...
      if [ -e $parent/config.mainnet.json ] ; then
        ...
        # echo "Break1"; pause
        if [ "$keys" == "Y" ]; then
          cp $parent/config.mainnet.json $benchcoreDir
          echo -e "\n$(green " ✔ Config was restored in $benchcoreDir")\n"
          read -e -r -p "$(yellow " Do you want to start BenchCore Node now? (Y/N): ")" -i "Y" keys
          if [ "$keys" == "Y" ]; then
            start
          fi
        else
...
three() {
  ...
  if [ "$UP_TO_DATE" -ne 1 ]; then
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
      ...
      echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "       Directory: $benchcoreDir")\n"
      echo -e "\n$(green "             Updating BenchCore Node...")\n"
      update_benchcore
      echo -e "$(green "                Restarting...")"
      forever restart $forever_process >&- 2>&-
      echo -e "\n$(green "    ✔ BenchCore Node was successfully restarted")\n"
      pause
    else
      echo -e "\n$(red "       ✘ BenchCore Node process is not running")\n"
      echo -e "$(green "            Updating BenchCore Node...")\n"
      update_benchcore
      forever start app.js --genesis genesisBlock.mainnet.json --config config.mainnet.json >&- 2>&-
      echo -e "$(green "    ✔ BenchCore Node was successfully started")\n"
      pause
    fi
  else
    echo -e "         $(igreen " BenchCore Node is already Up-to-date \n")"
    sleep 2
  fi
}
...
four() {
  ...
  echo -e "    $(ired "                                        ")"
  echo -e "    $(ired "   WARNING! This option will stop all   ")"
  echo -e "    $(ired "   running BenchCore Node processes, remove   ")"
  ...
  if [[ "$YN" =~ [Yy]$ ]]; then
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
      ...
      echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "       Directory: $benchcoreDir")\n"
      echo -e "\n$(green "            Stopping BenchCore Node...")\n"
      cd $benchcoreDir
      forever stop $forever_process >&- 2>&-
      echo -e "$(green "             Dropping BenchCore DB...")\n"
      drop_db
      drop_user
      echo -e "$(green "             Creating BenchCore DB...")\n"
      create_db

      # Here should come the snap choice
      snap_menu
      echo -e "$(green "            Starting BenchCore Node...")"
      forever start app.js --genesis genesisBlock.mainnet.json --config config.mainnet.json >&- 2>&-
      echo -e "\n$(green "    ✔ BenchCore Node was successfully started")\n"
      pause
    else
      echo -e "\n$(red "       ✘ BenchCore Node process is not running")\n"
      echo -e "$(green "             Dropping BenchCore DB...")\n"
...
five() {
  ...
  if [ "$keys" == "Y" ]; then
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
      echo -e "\n$(green "       Instance of BenchCore Node found with:")"
      echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "       Directory: $benchcoreDir")\n"
      echo -e "$(green "                Restarting...")"
      forever restart $forever_process >&- 2>&-
...
start() {
  ...
  if [ -e $benchcoreDir/app.js ]; then
    ...
    echo -e "\n$(green "       ✔ BenchCore Node installation found!")\n"
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
      echo -e "$(green " A working instance of BenchCore Node was found with:")"
      echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "   and Work Directory: $benchcoreDir")\n"
    else
      echo -e "$(green "            Starting BenchCore Node...")\n"
      ...
      sleep 1
      proc_vars
      echo -e "\n$(green "       BenchCore Node started with:")"
      echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "   and Work Directory: $benchcoreDir")\n"
    fi
  else
...
status() {
  ...
  if [ -e $benchcoreDir/app.js ]; then
    ...
    echo -e "\n$(green "       ✔ BenchCore Node installation found!")\n"
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
      echo -e "$(green "      BenchCore Node process is working with:")"
      echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "   and Work Directory: $benchcoreDir")\n"
    else
      echo -e "\n$(red "       ✘ No BenchCore Node process is running")\n"
...
killit() {
  ...
  if [ -e $benchcoreDir/app.js ]; then
    ...
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
      ...
      echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
      echo -e "$(green "   and Work Directory: $benchcoreDir")\n"
      echo -e "$(green "            Stopping BenchCore Node...")\n"
      cd $benchcoreDir
      forever stop $forever_process >&- 2>&-
      cd $parent
      echo -e "$(green "    ✔ BenchCore Node was successfully stopped")\n"
  else
    echo -e "\n$(red "       ✘ No BenchCore Node process is running")\n"
    fi
  else
...
log() {
  ...
  proc_vars
  trap : INT
  tail -f $benchcoreDir/logs/benchcore.log
  # pause
}
...
show_menus() {
  ...
  echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
  echo
  echo "              1. Install BenchCore"
  echo "              2. Reinstall BenchCore"
  echo "              3. Update BenchCore"
  ...
  echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
  echo
  echo "              A. BenchCore Start"
  echo "              R. Restart BenchCore"
  echo "              K. Kill BenchCore"
  echo "              S. Node Status"
  echo "              L. Node Log"
  echo "              0. Exit"
  echo
...
sub_menu() {
  ...
  echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
  echo
  echo "           1. Install BenchCore Cli"
  echo "           2. Install BenchCore Explorer"
...
if [ -e ./.firstrun ]; then
  ...
  echo -e "$(ired "    !!!  CHOOSE '1' TO INSTALL BenchCore NODE !!!    ") "
  exit
fi
